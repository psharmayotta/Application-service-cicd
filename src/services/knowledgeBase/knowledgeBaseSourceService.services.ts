...
import { KnowledgeBaseSourceModel } from '../../database/repository/knowledgeBaseSource/knowledgeBaseSource.model';
import { KnowledgeBaseSourceDto } from '../../database/repository/knowledgeBaseSource/knowledgeBaseSource.dto';
import { Pagination } from '../../core/InferParams';
import { MetaModel } from '../../core/MetaModel';

class KnowledgeBaseSourceService extends BaseServices {
    constructor(
        entity: any = KnowledgeBaseSourceEntity,
        protected awsService: AwsService = new AwsService()
    ) {
        super(entity, awsService);
    }

    getModel(): KnowledgeBaseSourceModel {
        return new KnowledgeBaseSourceModel();
    }

    getDTO(): any {
        return KnowledgeBaseSourceDto;
    }
    ...
    getMetaModel(): MetaModel {
        return new MetaModel('knowledge_base_source', 'icon', [
            {
                fileKey: "icon",
    ...
    override async transformFileData(model: KnowledgeBaseSourceModel, files: any): Promise<any> {
        try {
            let fileData = null;
    ...
    override postProcessAfterGetData(result: any, param: Pagination): Promise<any> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                console.log('-------KnowledgeBaseSourceService postProcessAfterGetData--', error);
                reject(error);
            }
        });
    }
        ...
        try {
            const query = this.entity
                ...
                .select([
                    ...
                    'rs.modified_at as modified_at',
                ]);

            if (param.is_delete !== undefined) {
                query.andWhere('rs.is_delete = :isDelete', { isDelete: param.is_delete });
            } else {
                query.andWhere('rs.is_delete = :isDelete', { isDelete: 0 });
            }

            if (param.search_text) {
                const searchText = `%${param.search_text.toLowerCase()}%`;
                query.andWhere(
                    '(LOWER(rs.name) LIKE :search OR LOWER(rs.description) LIKE :search)',
                    { search: searchText }
                );
            }
            ...
            const countQuery = this.entity.createQueryBuilder('rs');

            if (param.is_delete !== undefined) {
                countQuery.andWhere('rs.is_delete = :isDelete', { isDelete: param.is_delete });
            } else {
                countQuery.andWhere('rs.is_delete = :isDelete', { isDelete: 0 });
            }
...
export default KnowledgeBaseSourceService;
